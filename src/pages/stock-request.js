  ...
  const approvePunchOrder = (orderId, status) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post( process.env.REACT_APP_ADMIN_URL + 'approvePunchOrder.php', {orderId, status:1, userId:userDetails.id} ).then(function(response){
          var data = response.data;
          if(data.statusCode === 200){
        ...
        axios.post( process.env.REACT_APP_ADMIN_URL + 'approvePunchOrder.php', {orderId, status:2, userId:userDetails.id} ).then(function(response){
          var data = response.data;
          if(data.statusCode === 200){
            notify("success",data.msg);
          }else if(data.statusCode === 201){
