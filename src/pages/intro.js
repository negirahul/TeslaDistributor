...
function Intro() {
  ...
  function getUsers() {
    axios.get( process.env.REACT_APP_ADMIN_URL + 'user-detail.php?userId='+cookies.userId+'&userType=3').then(function(response) {
  ...
  return (
    ...
    <Link className="nav-Link open-link btn-black text-center" to={'/login'}>START AS DISTRIBUTOR <ArrowRight /></Link>
      </div>
    </div>
  );
}
